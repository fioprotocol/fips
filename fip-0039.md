...
* Enabling of _newaccount_ action. It is a system action now, but could have been made public. However, that approach was deemed more complex as it would require updates to other smart contracts which are calling this action now.
* Storing the encryption key as a new _token_code_ for _chain_code:FIO_ using _add_pub_address_. However, it would have been harder to support backwards compatibility where the original key is returned when a user has not specified a new encryption key.
