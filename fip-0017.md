...
|fio.wrap|wraptokens||Transfers FIO Tokens to designated FIO account on the way to another chain.|
|fio.wrap|unwraptokens||Transfers FIO Tokens to designated FIO account back from another chain.|
...
# Specification
## Overview
This FIP proposes a new fio.wrap system contract (controlled by eosio account or block producers). It will allow pre-defined oracles to transfer FIO Tokens or Domains owned by the smart contract if all oracles agree to do so.
...
* Oracle fee is transferred from actor account to fio.wrap.
* Chain wrap_fio_domain fee is collected.
* Owner of domain is changed from current account to fio.wrap.
...
### Unwrap FIO Domain
Transfers FIO Domain from fio.wrap to designated FIO account.
#### Contract: fio.wrap
...
#### Processing
* Request is validated per Exception handling.
* Owner of domain is changed from fio.wrap to account which hashes down from FIO Public Key mapped to supplied FIO Address. If does not exist, it is created.
* Action explicitly not taken:
  * Domain expiration date is not updated
  * is_public flag is not updated
#### Exception handling
...
|Not owner of FIO Domain|fio.wrap does not own the domain|403|||Type: invalid_signature|
#### Response body
|Parameter|Format|Definition|
|---|---|---|
|status|String|OK if successful|
...
* Tokens are transferred to fio.wrap.
#### Exception handling
|Error condition|Trigger|Type|fields:name|fields:value|Error message|
|---|---|---|---|---|---|
...
### Unwrap FIO Tokens
Transfers FIO Tokens from fio.wrap to designated FIO account.
#### Contract: fio.wrap
...
#### Processing
* Request is validated per Exception handling.
* *amount* is transferred from fio.wrap to account which hashes down from FIO Public Key mapped to supplied FIO Address. If does not exist, it is created.
#### Exception handling
|Error condition|Trigger|Type|fields:name|fields:value|Error message|
|---|---|---|---|---|---|
|Invalid amount|Transfer amount is not valid.|400|"amount"|Value sent in, e.g. "-1"|"Invalid amount"|
