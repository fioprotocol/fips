...
# Specification
## Overview
This FIP proposes a new wrapping system contract to be controlled by eosio account (BPs). It will allow pre-defined oracles to transfer FIO Tokens or Domains owned by the smart contract if all oracles agree to do so.

### Use cases
### Registering oracles
* Each oracle is registered using *regoracle* action
* Only BPs can register oracles via an msig
...
* Oracles mint tokens on another chain (not covered by this FIP):
  * Oracles monitor every block looking for *wraptoken* action
