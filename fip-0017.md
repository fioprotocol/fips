...
|---|---|---|---|
|fio.wrap|wrapdomain|wrap_fio_domain|Transfers FIO Domain to designated FIO account on the way to another chain.|
|fio.wrap|unwrapdomain|unwrap_fio_domain|Transfers FIO Domain to designated FIO account back from another chain.|
|fio.wrap|wraptokens|wrap_fio_tokens|Transfers FIO Tokens to designated FIO account on the way to another chain.|
|fio.wrap|unwraptokens|unwrap_fio_tokens|Transfers FIO Tokens to designated FIO account back from another chain.|
|fio.wrap|regoracle||Registers an Oracle.|
|fio.wrap|unregoracle||Unregisters an Oracle.|
|fio.wrap|setoraclefee||Allows Oracle to set Oracle fee.|
...
|max_oracle_fee|Positive Int|Maximum amount of SUFs the user is willing to pay for fee. Should be preceded by /get_oracle_fees|
...
* Oracle fee is transferred from actor account to all registered oracles in even amounts.
* Chain wrap_fio_domain fee is collected.
* Owner of domain is changed from current account to wrap contract account
...
|Insufficient balance|Balance is less that oracle fee + chain fee|400|"max_oracle_fee"|Value sent in, e.g. "100000000000"|"Insufficient balance"|
...
|Insufficient balance|Balance is less that amount + oracle fee + chain fee|400|"amount"|Value sent in, e.g. "100000000000"|"Insufficient balance"|
|Invalid FIO Public Key|FIO Public Key format is not valid|400|"fio_public_key"|Value sent in, e.g. "notakey"|"Invalid FIO Public Key"|
|Invalid oracle fee value|max_oracle_fee format is not valid|400|"max_oracle_fee"|Value sent in, e.g. "-100"|"Invalid oracle fee value"|
|Oracle fee exceeds maximum|Actual oracle fee is greater than supplied max_oracle_fee|400|"max_oracle_fee"|Value sent in, e.g. "1000000000"|"Oracle fee exceeds supplied maximum"|
|Invalid fee value|max_fee format is not valid|400|"max_fee"|Value sent in, e.g. "-100"|"Invalid fee value"|
...
|Error condition|Trigger|Type|fields:name|fields:value|Error message|
|---|---|---|---|---|---|
|Invalid oracle|Oracle is not valid|400|"oracle_actor"|Value sent in, e.g. "alice"|"Invalid oracle"|
...
|Error condition|Trigger|Type|fields:name|fields:value|Error message|
|---|---|---|---|---|---|
|Invalid oracle|Oracle is not registered for this wrapping contract|400|"oracle_actor"|Value sent in, e.g. "alice"|"Invalid oracle"|
|No authority|The signer does not have authority to register oracles|403|||Type: invalid_signature|
#### Response body
|Parameter|Format|Definition|
|---|---|---|
...
|Parameter|Required|Format|Definition|
|---|---|---|---|
|wrap_fio_domain|Positive Int|Amount of SUFs the oracle is requesting for wrapping FIO Domain.|
|wrap_fio_tokens|Positive Int|Amount of SUFs the oracle is requesting for wrapping FIO Tokens.|
|actor|Yes|12 character string|Valid actor of signer|
##### Example
```
{
...
An approach was considered to make the contract generic, so that anyone could create a wrapping system, but configuring that generic contract and building their own oracles and smart contracts on another chain. This approach was deemed too complex to consider at this stage, but could be revisted in the future. In the meantime anyone can submit their own FIP to deploy a new wrapping contract to the FIO Chain.
