...
* If FIO handle is already registered and actor is validated, then check if url, producer_key or url parameters against producer record
* If producer public key (validated), url or location are different, update their records in the producer table
